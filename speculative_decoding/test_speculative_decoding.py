...
def base_decoding(
    ...
    for _ in range(sample_num_times):
        if use_cache:
            ...
            logits = net(out, return_cache = False) # logits['logits'].shape = (batch, prompt_seq_len, vocab_size)
        try:
            logits = logits[:, -1]
        except: 
            # TypeError: tuple indices must be integers or slices, not tuple
            # AttributeError: 'NemotronHCausalLMOutput' object has no attribute 'shape'
    ...
    while (seq_lens < seq_len).any():
        ...
        for _ in range(gamma):
            ...
            try:
                ...
                # TypeError: tuple indices must be integers or slices, not tuple
                # AttributeError: 'NemotronHCausalLMOutput' object has no attribute 'shape'
                small_logits = small_logits['logits'][:, -1]
...
DEVICE_STR = 'cuda:0' # control using CUDA_VISIBLE_DEVICES
...
# Note:
# forward takes in x, return_loss = False, return_cache = False, seq_start_pos = None, cache = None, early_exit_cache = None, return_early_exit_only = False, start_from_early_exit_hiddens = False
# forward returns (logits), or (logits, cache), or (loss), or (loss, early_exit_loss)
...
nemo = {"large": "nvidia/Nemotron-H-8B-Reasoning-128K", "small": "nvidia/NVIDIA-Nemotron-Nano-9B-v2"}
# check if this makes sense, small model has more params than large. but large has been working

tokenizer  = AutoTokenizer.from_pretrained(nemo["small"], trust_remote_code=True)
modellarge = AutoModelForCausalLM.from_pretrained(nemo["large"], torch_dtype=torch.bfloat16, trust_remote_code=True).to(device)
modelsmall = AutoModelForCausalLM.from_pretrained(nemo["small"], torch_dtype=torch.bfloat16, trust_remote_code=True).to(device)

prompt = "When was NVIDIA founded?"
prompt = tokenizer(prompt, return_tensors="pt", add_special_tokens=False).to(device)
prompt = prompt['input_ids']
import time
start_time = time.time()

sampled, base_decode_elapsed = benchmark(base_decoding)(modellarge, prompt, GENERATE_LENGTH, use_cache=False)

(spec_decode_sampled, num_accepted), spec_decode_elapsed = benchmark(speculative_decoding)(modellarge, modelsmall, prompt, GENERATE_LENGTH, GAMMA, use_cache=False)


# base_decode_output = decode_tokens(sampled[0])
# spec_decode_output = decode_tokens(spec_decode_sampled[0])
base_decode_output = tokenizer.decode(sampled[0], skip_special_tokens=True)
spec_decode_output = tokenizer.decode(spec_decode_sampled[0], skip_special_tokens=True)
